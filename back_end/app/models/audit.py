...
class Audit(BaseModel):
    ...
    Conforme à l'architecture BaseModel pour cohérence.
    """
    ...
    # Foreign keys avec référence correcte à users.userid (pas users.id)
    user_id = db.Column(db.String(36), db.ForeignKey('users.userid'), nullable=False, index=True)
    consent_id = db.Column(db.String(36), db.ForeignKey('consent_logs.id'), nullable=True)
    
    # Relation bidirectionnelle avec back_populates
    user = db.relationship('User', back_populates='audits')
    
    def __init__(self, target, user_id, consent_id=None):
    ...
    def to_dict(self):
        """Retourne une représentation dictionnaire"""
        return {
            'id': self.id,
            'target': self.target,
            'status': self.status,
